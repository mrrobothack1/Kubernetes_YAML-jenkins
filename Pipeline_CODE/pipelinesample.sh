...
def scm_repos="git@github.soju.com:StoreRepos/video-ml-configs"
def scm_repo="git@github.soju.com:cx/video-ml"
...
pipeline {
    ...
    stages {
        ...
        stage('Maven Build') {
            steps {
               sh """
                  mvn clean install  -Dmaven.test.skip=true sonar:sonar -Dsonar.host.url=http://nwc00887.cp.ad.soju.com:9000
               """
            }
        }
        /*stage('contents_of_the_dir') {
            steps {
                sh """
                   ls -lrt
                   cd mlsi-frontend/target/ && ls -lrt
                """  
            }
        }*/
        stage('frontend-app building') {
            steps {
                sh """
                   cd mlsi-frontend/
                   npm install
        ...
        stage('Nexus Upload') {
            steps {
               sh """
                 cd ${WORKSPACE}/mlsi-backend/target/
                 curl -v -u Mobile_Admin:M0b1l1ty --upload-file *.jar  http://10.8.43.107:8080/nexus/content/repositories/snapshots/com/soju/mobile/service/${dimg_version}-${env_name}-${app_branch_name}_${app_name}.jar
               """
            }
        }
        stage('BuildConfig') {
            steps {
               ...
               sh """
                  ...
                  sed -i "s/BRANCH_NAME/${dimg_version}-${env_name}-${app_branch_name}_${app_name}/g" Dockerfile    
